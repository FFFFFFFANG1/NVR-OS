...
typedef union idt_desc_t {
    ...
    struct {
        ...
        uint8_t  reserved4; // unused, set to 0
        uint32_t reserved3 : 1; // 1 if TRAP gate and 0 if INTR gate
        uint32_t reserved2 : 1; // set to 1
        uint32_t reserved1 : 1; // set to 1
        uint32_t size      : 1; // set to 1
        uint32_t reserved0 : 1; 
        uint32_t dpl       : 2; // 2 bit show the privilege level
        uint32_t present   : 1; // 1 if used
